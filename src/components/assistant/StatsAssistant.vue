<template>
  <Teleport to="body">
    <button v-if="!open" ref="launcher" class="owcs-assistant-launcher" type="button" @click="show" aria-label="打开赛事助手">
      <span class="assistant-mark" aria-hidden="true">✦</span><span>问问比赛</span>
    </button>
    ...
    <section v-if="open" ref="panel" class="owcs-assistant" role="dialog" aria-label="赛事助手" @keydown.esc="close" @keydown="trapFocus">
      ...
      <div v-if="serviceUnavailable" class="assistant-service-note">请启动本机助手服务，或打开<router-link to="/data-manage/assistant" target="_blank" rel="noopener">模型设置</router-link>。</div>
    </section>
  </Teleport>
</template>
...
import { useCurrentAssistantPage } from '@/services/assistantContext';
...
<style scoped>
.owcs-assistant-launcher,.owcs-assistant{--a-ink:#252938;--a-muted:#687488;--a-line:#e1e5ec;--a-accent:#c75113;font-family:Inter,"Microsoft YaHei",sans-serif;color:var(--a-ink)}
.owcs-assistant-launcher{position:fixed;right:24px;bottom:24px;z-index:2500;display:flex;align-items:center;gap:10px;border:1px solid #e2e5eb;border-radius:28px;background:#fff;padding:12px 18px;box-shadow:0 6px 28px #22304722;font-size:14px;font-weight:700;cursor:pointer}.assistant-mark{color:var(--a-accent);font-size:24px;line-height:1}
...
@keyframes assistant-pulse{50%{opacity:.25}}@media(prefers-reduced-motion:reduce){.assistant-progress i{animation:none}}@media(max-width:600px){.owcs-assistant-launcher{right:14px;bottom:calc(18px + env(safe-area-inset-bottom));padding:11px 15px}.owcs-assistant{width:100%;right:0;bottom:0;height:88dvh;border-radius:18px 18px 0 0;padding-bottom:env(safe-area-inset-bottom)}.assistant-mobile-shade{display:block;position:fixed;inset:0;background:#17233555;z-index:2999}.assistant-header{padding:14px 18px}.assistant-messages{padding:18px}.assistant-composer textarea{font-size:16px}}
</style>
