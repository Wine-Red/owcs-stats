...
配置和加密密钥仅写入 `.local/assistant/`，已被 Git 忽略。旧版本留下的 `admin-token.txt` 不再读取或使用，新建配置也不生成它。密钥与加密配置位于同一用户目录，加密用于避免明文落盘，不能防护已取得本机目录权限的程序。访客聊天没有登录、账号或服务端聊天记录。浏览器只保留内存中的临时对话，刷新清空；发问时将最近对话和当时页面范围发送给服务。模型供应商会收到这些内容，供应商自己的保留策略不由本服务控制。
