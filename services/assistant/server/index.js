...
const stateDirectory = process.env.ASSISTANT_STATE_DIR || path.join(root, '.local/assistant');
const settings = createSettings(stateDirectory);
const conversations = createConversations(stateDirectory);
const retentionTimer = setInterval(() => { try { conversations.prune(); } catch { console.warn('assistant_retention_failed'); } }, 3600000);
...
const app = createApp({ settings, conversations, client, wiki: createWiki(), production, publicOrigin,
