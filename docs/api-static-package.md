...
- 按普通页面使用：打开、导航或浏览器刷新时按需读取数据。停留期间不轮询修订、不提示更新，也不自动重载内容；回到前台或恢复网络不会触发重载。后台数据变更无需重新发布前端包，重新打开或刷新页面即可读取。
- 页面请求遇到断网、超时、服务异常时显示重试入口，不把旧快照或空数据伪装成实时成功结果。`/meta` 保留供接口检查和其他客户端读取来源状态，当前页面包不进行后台检查。
- 时效受既有同步和来源缓存影响：数据库同步与赛程源通常按 5 分钟更新，展示缓存最多再增加 15 秒。不保证跨多个 HTTP 请求的原子快照；需要固定一致数据时使用纯静态包。
...
`site-package.config.json` 是维护者的统一配置源，构建时校验并内嵌，同时输出 `site-config.json` 和 `package-manifest.json` 供核对。运行不请求这些旁置 JSON；直接修改旁置 JSON 不会生效。修改超时、路径或源地址时，以 `OWCS_SITE_CONFIG` 指向新配置重构建，内嵌配置和 CSP 自动同步，无需改前端源码。公开服务使用 HTTPS，HTTP 仅允许本机预览。
