...
`export-static.mjs` 只消费快照格式，发现并本地化媒体，输出带内容摘要的资源文件及清单。它不维护页面请求列表，也不通过浏览器爬取页面。中间数据按赛事/选手等实体分文件、时间线按地图局分文件；交付构建将这些记录压缩内嵌到 HTML，访问时才解压并读取。
...
解压后运行只需上传 `dist/index.html`；`package-manifest.json`、`static-data/manifest.json` 和 `DEPLOY.txt` 是核对材料，运行不会请求它们。代码、样式、字体、图片和压缩数据都在 HTML 内，支持域名根目录和子目录及 Hash 路由刷新。页面不向合作方资源 CDN 发请求，CSP 限制对外部服务的主动连接。图片按需生成 Blob URL，支持 canvas 导出。普通资料来源链接仍可由用户主动打开。

HTML 使用 `Cache-Control: no-cache`，建议启用 gzip/Brotli 传输。平台需保留内联代码及对应 CSP 哈希，不要再次拆成跨域 ES Module。首次 HTML 较大，数据按资源解压而非一次性解析全部统计记录。`npm run verify:partner-host` 通过真实的页面域/无 CORS CDN 对照测试验证此场景，并已加入 Release。

更新时上传新的完整 HTML，校验后切换，保留上一版以便回滚；不要修改其中的部分压缩资源或脚本。正在浏览旧页面的用户在切换后可能需要刷新。后续如需要独立的数据更新包，再基于数据格式兼容性和资源保留策略扩展。
