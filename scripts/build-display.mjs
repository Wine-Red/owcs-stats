...
await writeFile(`${output}/package-manifest.json`, `${JSON.stringify({ schemaVersion: 1, mode: live ? 'api' : 'snapshot',
  ...
  configFile: live ? 'site-config.json' : undefined, capabilities: { readOnly: !interactions.voting && !interactions.assistant, voting: !!interactions.voting, assistant: !!interactions.assistant, bundledData: !live } }, null, 2)}\n`);
